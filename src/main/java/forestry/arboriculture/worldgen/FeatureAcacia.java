...
public class FeatureAcacia extends FeatureTree {
	...
	public void generateTrunk(LevelAccessor level, List<BlockPos> logOrigins, List<BlockPos> branchCoords, RandomSource rand, TreeBlockTypeLog wood, BlockPos startPos) {
		...
		if (!firstDir.equals(nextDir)){
			FeatureHelper.generateTreeTrunk(level, branchCoords, rand, wood, startPos, rand.nextIntBetweenInclusive(Math.max(2, height-4), height), girth, 0, 0, nextDir, (rand.nextFloat()*1.5f)+0.5f);
		}
	}
